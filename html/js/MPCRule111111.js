    ...
    predict = function (lastRequested, lastQuality, lastHTTPRequest, last_Bufferleveling) {//lastRequested==index
        ...
        if (past_Throughput.length == 0) {
           ...
           /*if (lastRequested < 6) {
            ...
            let predict_band = tmpDownloadTime/tmpSum;
            initial_Bandwidthpredict=predict_band;
            console.log(" initial_Bandwidthpredict", initial_Bandwidthpredict);
            ...
            if (past_Bufferlevel.length === 0) {
                ...
                let bufferIndex = Math.max(0, past_Bufferlevel.length - horizon);
                let bufferSum = 0;
                for (var i = bufferIndex; i < past_Bufferlevel.length; i++) {
                    bufferSum = bufferSum + past_Bufferlevel[i];
                }
            ...
            for (let i = 0; i < 6; i++) {
                for (let j = 0; j < 6; j++) {
                    ...
                    qoe = VIDEO_BIT_RATE[j] / 1000 - beta * [VIDEO_BIT_RATE[j] / 1000 - VIDEO_BIT_RATE[last_quality] / 1000] - deta * rebuffer;
                    if (j == i)
                        QoE_max = qoe;
                    else
                        qoe_log.push(qoe);
